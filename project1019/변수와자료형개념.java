...
(5) 데이터가 메모리에 올라가는 시점은 프로그램 소스 파일을 저장할 때이다.
	x : 이 세상의 모든 프로그램에서 메모리에 데이터가 올라오는 경우는 오직 실행할때만 이다! 컴파일 할때도 아님
...
(4) 컴퓨터의 메모리에는 문자 자체가 올라갈 수 없으므로 사실상 자바언어에서 사용되는 char형은 숫자 자료형
으로 봐도 무관하다.
o : 모든 프로그래밍 언어에서 문자는 사실상 숫자이다!!!
...
x : 쓰지 않는 이유는 클래스랑 혼동되기 때문이다! 다른 개발자와 협업 위함
