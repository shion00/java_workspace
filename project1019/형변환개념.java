...
x : 보관만이 목적이라면 가능하겠으나, 연산을 수행하면 자동형변환이 일어나기 때문에  
	 int 보다 작은 자료형을 쓰는 것은 의미 없는 짓이다.
(4) (마)에서 7의 결과가 담아지므로, 에러는 발생하지 않는다.
x : 정수의 연산수행에서 연산의 대상이 되는 데이터가 int 형보다 작은 자료형일 경우(byte,short) 
	 연산속도를 높이기 위해, 연산에 최적화된 자료형인 int 형으로 자동으로 형변환을 일으킨다. (자동형변환)
	 int의 4byte 보다 작은 short의 2byte 에 담는 경우 에러가 발생한다.
...
o : 속도보다 데이터 손실을 막는게 중요하다, 둘다 long으로 바뀐다.
...
x : 보관만(로드만)을 목적으로 사용한다.
(3) float  x = 3.14;  라고 선언시 3.14는 실수이므로 정상적으로 컴파일 될 것이다.
x : 디폴트가 double, 8byte에서 작은 float의 4byte에 담으려 하기 때문에 오류가 난다.
	 해결) float x=3.14f;
(4) 자바에서 실수는 무조건 double 형으로 표현된다.
o : 디폴트가 double이다, 따라서 float 쓸때는 f를 붙여야 한다.
...
x : int로 받아야 한다. int 형보다 작은 자료형일 경우(byte,short) 
	 연산속도를 높이기 위해, 연산에 최적화된 자료형인 int 형으로 자동 형변환을 일으킨다. 
...
(1) 위 코드의 (가) 부분에서 a는 일단 +연산의 수행 전 int 형으로 변경된다.
 x : 큰 쪽으로 바껴야 데이터 손실이 발행하지 않으므로 long 으로 변경된다.
